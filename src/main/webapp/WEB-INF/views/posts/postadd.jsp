	   ...
	   for(let i=0; i<inputConvenient.length; i++)
	    {
	      inputConvenient[i].onclick = function() {
	        if(inputConvenient[i].checked === true) {
	         inputConvenient[i].parentElement.querySelector("label").classList.add("label-checked-active");
	        ...
	        if(inputConvenient[i].checked === false) {
	         ...
	         inputConvenient[i].parentElement.querySelector("label").classList.remove("label-checked-active");
	        }
	      }
	      
	    }

	   const imgLoadList = document.querySelectorAll(".content-img-block-label label");
	   for(let i = 0; i < imgLoadList.length; i++) {
	       imgLoadList[i].onclick = function() {
	           console.log(imgLoadList[i].parentElement.querySelector("input"));
	           imgLoadList[i].parentElement.querySelector("input").onchange = function() {
	              imgLoadList[i].querySelector(".img-wait-load").classList.add("none-block");
	              imgLoadList[i].querySelector(".img-done-load").classList.remove("none-block");
	           } 
	        }    
	   }
